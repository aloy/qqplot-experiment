...
Various procedures have been proposed to test whether a random sample comes from a normal distribution. In this section we review commonly used tests of normality. 

Numerous distributional tests focus on the difference between the empirical and theoretical distribution functions. More formally, let $F_n$ be the empirical distribution function (ECDF) based on a sample of size $n$, and $F$ be the hypothesized/true distribution. The absolute difference between the two distribution functions for each sample point, $\left| F_n(x_i) - F(x_i) \right|$, is the main contributor for the test statistics of the Kolmogorov-Smirnov \cite[KS-test,][]{kolmogorov:1933, smirnov:1948}, the Lilliefors \cite[LF-test, ][]{lilliefors}, the Anderson-Darling \citep[AD-test,][]{adtest:1954}, and the Cram\'{e}r-von-Mises tests \citep[CvM-test,][]{cramer:1928, mises:1928}, as shown in Table~\ref{tab:tests}.
...
regardless of the range of the sample---i.e., a difference, $D$, observed in either tail of the distribution carries the same weight and is interpreted in the same way as a difference, $D$, in the center of the distribution. While the KS test allows for the adjustment of the parameters of the normal distribution to the sample mean and variance, it is more appropriate to use the LF test for this purpose. LF and KS share the same test statistic, but the sampling distribution of the LF test statistic is adjusted for the two additional parameters.  AD and CvM  are both based on the total area between the hypothesized distribution function and the empirical distribution function. Compared to the KS  test,  the CvM test downplays the effects in the tails of a (normal) distribution, while the AD test emphasizes the tail effect using a weighting of $1/\left(F(x)(1 - F(x)\right)$ across the range of the sample. 
...
For any sample tested against a distribution within a location-scale family, such as a normal, log normal, or exponential distribution, the sample quantiles still fall on a line when plotted against the theoretical quantiles of any of the family's member distributions. For example, plotting the empirical quantiles of a normally distributed sample $x \sim N(\mu, \sigma^2)$ against the quantiles of a standard normal will result in a line, where  the slope is an estimate of $\sigma$, and the intercept estimates $\mu$. Visually  the only change in the Q-Q plot is a  change in the scale of the $y$-axis. We can therefore employ Q-Q plots in the more general framework of testing the distribution of a sample for normality similar to standard normality tests, such as the AD, LF, CvM, and SW tests. 
%However, we do have to make a decision with respect to the exact parameters of the normal distribution we test against when we plot a line alongside the points in the Q-Q plot for additional comparison purposes---i.e.~the parameters $\mu$ and $\sigma$ have to be estimated from the sample. 
However, in order to draw a line alongside the points of a  Q-Q plot, we have to estimate the parameters $\mu$ and $\sigma$ from the sample. 


In Q-Q plots, variability is based on a robust measure of spread given as the ratio of the inter-quartile ranges (IQRs) of the empirical and theoretical distributions:\\ $\left(F^{-1}_n(0.75) - F^{-1}_n(0.25)\right) / \left(F^{-1}(0.75) - F^{-1}(0.25)\right)$ \citep{becker:s}. 
...
Following this recommendation, we are investigating the power of detrended Q-Q plots. In particular
...
\widehat{\text{Power}} = \text{Power}_{N} = 1 - F_{N} (y_{\alpha}),
\end{equation}
...
We expect the performance on the lineups to reflect the deviation from normality in the distribution; thus, lineup performance should decrease as the degrees of freedom increase, and the corresponding $t$-distribution approaches the normal distribution. Similarly, with decreasing sample size the difference between a $t$- and a normal distribution is more pronounced, which should be reflected by an increase in sample identifications  in the lineup evaluations.   
...
Null samples are drawn from $N(0, S^2)$; the reference line has a slope of $S$ (and an intercept of 0).  All panels have the same scale. Throughout the paper we use $\widehat{\sigma}^2$ to indicate the sample variance $\widehat{\sigma}^2 = (n-1)^{-1} \Sigma_{i=1}^n (x_i - \bar{x})^2$, whereas $\widehat{S}^2$ is a more robust estimator based on the interquartile range.
\end{itemize}
...
Crowdsourcing graphical perception experiments through MTurk is a relatively new practice, and involves recruiting human observers from the web. While potential sources of bias in social science research using MTurk have been discussed \cite[see, for example,][]{chandler:2014}, the results from MTurk studies for graphical perception experiments are promising, as MTurk experiments have replicated studies conducted in the traditional lab setting \citep{Kosara:2010, Heer:2010}. Specifically, \citet{Heer:2010} replicated the results of \citet{cleveland:1984} using the MTurk platform, and \citet{Kosara:2010} found similar results between a lab study and one performed using MTurk. \citet{Kosara:2010} and \citet{Heer:2010} suggest the use of easy tasks to filter out participants  trying to ``game the system'' by completing the tasks without regard to the directions to receive their reward as quickly as possible. We have found that the impact of these individuals on results is sufficiently small when we adjust results for individuals' abilities in the evaluation. Also, we only accept participants that we believe will complete the task to the best of their ability.  In particular, for this study only participants who had (a) successfully completed at least 100 (other) tasks on MTurk, (b) a completion rate of 95\% in these tasks, and (c) successfully completed a mini training session using (easy) trial lineups were given access to the study. None of this  trial information is used in the evaluation of results.
...
Generally, within each panel, lines of the same color/shape (lineups that share the same data sample, but have different nulls) are closer together than lines of different color/shape (different sample, different nulls).  
...
%The difference in significance between normality tests and the visual test might be due to the way the theoretical distribution against which the sample is compared is chosen. 
The difference in results between the classical tests and the visual test may be due to the different estimation procedures for $\mu$ and $\sigma$.
...
In this section, we further investigate the differences between different Q-Q plot designs  found in Figure~\ref{fig:compare}.
...
On the left, estimates of the coefficients of the logistic regression model stated in \eqref{eq:model} are shown with 95\% confidence intervals given in the parentheses. For convenience, the corresponding odds, again,  with 95\% confidence intervals, are presented on the right. All significant values are bolded. Estimates of linear logistic regressions preserve their linear property---i.e.~we can easily infer that a $t_5$ sample of size 50 shown in an adjusted detrended Q-Q plot with a DH confidence band, has an estimated log odds of $0.93 = -4.84 + 0.42 + 3.00 + 2.35$, which corresponds to an estimated probability of $0.717 = e^{0.93}/(1 + e^{0.93})$ that the sample is identified as the most different in a lineup evaluation. For odds, linear relationships are substituted by multiplicative ones, but are more directly interpretable in terms of the response. For example, we can say that the probability of  a sample to be identified as non-normal in a lineup is 1.5 times higher, if the sample is shown in an adjusted detrended Q-Q plot with a DH confidence band  than if the same sample is shown in a control chart.
As was previously observed, an increase in signal strength (larger sample size, fewer degrees of freedom) results in lineups in which the sample is picked out more often by observers.
...
% print(xtable(dc))
\begin{table}[ht]
\caption{\label{tab:coefs} Linear estimates and corresponding odds based on Model \eqref{eq:model}.  95\% Wald confidence intervals are given in parentheses. Significant values are bolded. For parameters serving as baselines, no confidence intervals exist.}
\centering
...
To further investigate the difference between the designs, consider Figure \ref{fig:rotstd}. Here, we consider a sample from the $t_2$ distribution of size $n=20$ that is rejected by all designs besides the ordinary detrended  Q-Q plots with a DH band.  
The ordinary detrended  DH band is shown on the left side of Figure~\ref{fig:rotstd}. Instead of the data sample in panel \#$(2^2+1)$ observers picked panel \#7 in 32.2 out of 45 evaluations. We display the ordinary detrended  Q-Q plot with TS bands on the right side of Figure~\ref{fig:rotstd}, based on which observers picked the true plot at a rate that lead to a rejection of normality.
...
In the lineup on the left the data sample in panel \#$(2^2+1)$ is only identified 2.8 times in 45 evaluations in the lineup showing the ordinary detrended  design with a DH confidence band. Observers instead pick panel \#$(3^2-2)$ 32.2 times. The lineup on the right shows the same data in a ordinary detrended  Q-Q plot design using the TS confidence band. The data sample is picked 7.2 times in 27 evaluations, leading to a rejection of normality at a 5\% signficance level.}
\end{figure}
\afterpage{\clearpage}
...
In the comparison of the four Q-Q plot designs, adjusted detrended Q-Q plots, where the aspect ratio is kept the same as in the standard plot, are the most powerful. While at first it may seem counterintuitive that this version of detrending is most powerful, after realizing that the plotted distances between points in the ordinary detrended design are not proportional to distances in the data space, this difference in power is quite logical. Additionally, DH bands are slightly more powerful than TS bands, except for in the ordinary detrended design.
...
% Estimates of the variance components are $\widehat{\sigma}_u = round(m1@theta[1],2)$, $\widehat{\sigma}_d=round(m1@theta[2],2)$, and $\widehat{\sigma} = round(sqrt(sum((fitted(m1)-turkall$correct)^2)/(nrow(turkall)-8)),2)$. Variances of user ability and data difficulty are large relative to residual variance, indicating that both random effects are necessary.
% %
...
% require(lme4); eff <- range(ranef(m1)[["ip_address"]][,1]); probs <- exp(eff)/(1+exp(eff)); round(probs[2]/probs[1],1)-fold probability of detecting the data plot from a lineup. 
...
Under the standard design there is one lineup, the adjusted detrended  design has two lineups with different outcomes, while the ordinary detrended design has four lineups with different outcomes. They are shown in Figures~\ref{fig:lpsstds},~\ref{fig:lpsdt1}, and~\ref{fig:lpsdt2}, respectively. 
...
\end{knitrout}
\caption{\label{fig:lpsdt1} Lineups of adjusted detrended Q-Q plots with different outcomes depending on the choice of confidence band. The outlined panel marks the location of the data sample, the numbers at the bottom right are showing the number of votes each of the panels was picked by observers.  }
\end{figure}
\afterpage{\clearpage}
