...
\hh{Various} procedures have been proposed to test whether a random sample comes from a normal distribution. In this section we review commonly used tests of normality. 

\al{Numerous}  distributional tests focus on the difference between the empirical and theoretical distribution functions. More formally, let $F_n$ be the empirical distribution function (ECDF) based on a sample of size $n$, and $F$ be the hypothesized/true distribution. The absolute difference between the two distribution functions for each sample point, $\left| F_n(x_i) - F(x_i) \right|$, is the main contributor for the test statistics of the Kolmogorov-Smirnov \cite[KS-test,][]{kolmogorov:1933, smirnov:1948}, the Lilliefors \cite[LF-test, ][]{lilliefors}, the Anderson-Darling \citep[AD-test,][]{adtest:1954}, and the Cram\'{e}r-von-Mises tests \citep[CvM-test,][]{cramer:1928, mises:1928}, as shown in Table~\ref{tab:tests}.
...
\al{
However, \hh{in order to draw} a line alongside the points of a  Q-Q plot, \hh{we have to estimate} the parameters $\mu$ and $\sigma$ from the sample. 
}
...
\al{Following this recommendation,} we are investigating the power of detrended Q-Q plots. In particular
...
\widehat{\text{Power}} = \text{Power}_{N} = 1 - \al{F_{N}} (y_{\alpha}),
\end{equation}
...
\hh{We expect the performance on the lineups to reflect the deviation from normality in the distribution; thus, lineup performance should decrease as the degrees of freedom increase, and the corresponding $t$ distribution approaches the normal distribution. Similarly, with increasing sample size the difference between a $t$- and a normal distribution is more pronounced, which should be reflected in the lineup evaluation.   }
...
\alnote{How can we reword the above sentence?}
\alnote{Perhaps: The different results observed between the classical tests and the visual test may be due to the different estimation procedures for $\mu$ and $\sigma$.}
...
\alnote{How can we reword the above sentence?}
\alnote{Perhaps: In this section, we further investigate the differences between the Q-Q plot designs that were observed in Figure~\ref{fig:compare}.}
...
% print(xtable(dc))
\begin{table}[ht]
\caption{\label{tab:coefs} Linear estimates and corresponding odds based on Model \eqref{eq:model}.  95\% Wald confidence intervals are given in parentheses. \hh{Significant values are bolded. For parameters serving as baselines no confidence intervals exist.}}
\centering
...
To further investigate the difference between the designs, consider Figure \ref{fig:rotstd}. Here, we consider a sample from the $t_2$ distribution of size $n=20$ that is rejected by all designs besides the \al{ordinary} detrended  Q-Q plots with a DH band.  
The \al{ordinary} detrended  DH band is shown on the left side of Figure~\ref{fig:rotstd}. Instead of the data sample in panel \#$(2^2+1)$ observers picked panel \#7 in 32.2 out of 45 evaluations. We display the \al{ordinary} detrended  Q-Q plot with TS bands on the right side of Figure~\ref{fig:rotstd}, based on which observers picked the true plot at a rate that lead to a rejection of normality.
...
Under the standard design there is one lineup, the \al{adjusted} detrended  design has two lineups with different outcomes, while the \al{ordinary} detrended design has four lineups with different outcomes. They are shown in Figures~\ref{fig:lpsstds},~\ref{fig:lpsdt1}, and~\ref{fig:lpsdt2}, respectively. 
