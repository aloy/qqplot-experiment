...
\includegraphics[width=\maxwidth]{figure/nortests} 
...
\begin{figure}
\centering
\includegraphics[width=.3\textwidth]{figures/fig:visnorm.pdf}
\includegraphics[width=.6\textwidth]{figures/filebab632311351-multiple-svg.pdf}
\caption{\label{fig:visnorm} Scatterplot of $p$-values from Shapiro-Wilks and lineups of Standard Q-Q plots (left). The lineup plot of Standard Q-Q plots (right) corresponds to the observation circled on the left, which is highly significant under the lineup test, but not significant under Shapiro Wilks.  \hh{This is potentially problematic - the lineup flares up so strongly, because the lineup is testing against standard normal, while the normal tests are testing for the family - the KS test discussed here is the Lilliefors extension. The actual Ks test is significant for this dataset, but then only rejects 3 other.}}
\end{figure}
