...
\al{In statistical modeling we strive to specify models that \hh{resemble data collected in studies or observed from processes.} %could generate the observed data. 
\hh{Distributional specification and parameter estimation are central to parametric models.}
...
Graphical procedures, such as the quantile-quantile (Q-Q) plot \citep{Wilk:1968}, are arguably one of the most widely used method of distributional assessment, though critics find their interpretation to be overly subjective. Formal goodness-of-fit tests are available and are quite poweful, but only indicate whether there is a lack of fit, not why there is lack of fit. For example, the Shapiro-Wilk test \citep{Shapiro:1965kt} is a powerful test of normality, but does not indicate what feature of the distribution is non-normal, so a plot, such as a Q-Q plot, must be rendered after any rejection. 
...
\hh{We will first discuss  tests for normality, both from a numerically and graphically viewpoint, and then formally introduce the lineup protocol in the setting of Quantile-Quantile plots used for this paper.}
...
\hh{Based on a visual inspection in a Q-Q plot a}
 sample is \hh{therefore} considered to be consistent with a normal distribution if the empirical and theoretical quantiles fall close to \hh{the line representing the theoretical distribution}.  \hh{This decision is helped additionally by an assessment of}
...
\hhnote{Let's just leave it as is then. Thanks for checking!}
...
It is important to recall that none of the data plots in the lineups were actually created using data from a normal distribution. This should lead to rejection of the null hypothesis in every single instance \al{XXX Do we need to acknowledge type I error here?}.
This is not quite true, as can be seen in Table~\ref{tab:reject}, but what also becomes evident is the high power  of visual inference. Based on lineups we are able to reject non-normality much more often than with any of the classical tests.
...
The corresponding $p$-value the SW test is 0.2318, after LF (0.1689) the test with the lowest $p$-value on this data sample out of all the normality tests.
...
Q-Q plots are not restricted to the assessment of normality. In fact, they provide a general framework for testing \hh{any} distributional assumptions. Used in the setting of lineups, they in particular allow an assessment of \hh{limiting distributions, i.e.~for example, lineups allow us to investigate distributions of}
 samples from approximate normal or asymptotic normal distributions, \hh{for which  there exists  no classical test for finite sample sizes, whereas the lineup protocol provides us with a valid testing system}
  as long as there is a method  to generate data under the null hypothesis for creating null plots in the lineup.
 
\alnote{XXX the last sentence almost seemed limiting, maybe another sentence taking about other distribtutions would be valuable.}
\hhnote{it wasn't supposed to be limiting - I tried to shift the emphasis a bit ... does it work now?}
One of the \hh{drawbacks} of the lineup \hh{test framework} is that it comes at a higher cost, both monetary and in time, than classical testing.
...
The application of the lineup framework based on the related P-P plots poses a natural next question: \citet{koehler91} comment on the higher sensitivity of  P-P plots  to discrepancies in the middle of the distribution, such as caused by multiple modes. We can verify this and other statements for large samples and based on distributions. Lineups allow us to quantify the extent to which these statements hold for small sample sizes.
