...
<<abilities, dependson='model', echo=FALSE>>=
qplot(`(Intercept)`, geom="histogram", data=ranef(m0)$ip_address, binwidth=0.05) + xlab("Individuals' ability u")
qplot(`(Intercept)`, geom="histogram", data=ranef(m0)$difficulty, binwidth=0.5) + xlab("data difficulty level d")
...
<<rejects, dependson='data', echo=FALSE>>=
...
ggplot() + geom_abline(colour="grey50") +  geom_point(aes(x=Rotated, y=Standard, colour=df, shape=df),size=3, data=dt) + theme_bw() +scale_colour_brewer(palette="Set2") + theme(legend.position="none")
@
\end{subfigure}
\begin{subfigure}[b]{.3\textwidth}
<<dependson='triples', echo=FALSE>>=
ggplot() + geom_abline(colour="grey50") +  geom_point(aes(x=Rotated, y=Control, colour=df, shape=df),size=3, data=dt) + theme_bw() +scale_colour_brewer(palette="Set2") + theme(legend.position="none")
@
\end{subfigure}
\begin{subfigure}[b]{.3\textwidth}
<<dependson='triples', echo=FALSE>>=
ggplot() + geom_abline(colour="grey50") +  geom_point(aes(x=Control, y=Standard, colour=df, shape=df),size=3, data=dt) + theme_bw() +scale_colour_brewer("degrees of\nfreedom",palette="Set2") + theme(legend.position=c(0.85, 0.20), # c(0,0) bottom left, c(1,1) top-right.
  legend.background = element_rect(fill = "white", colour = NA)) + scale_shape("degrees of\nfreedom")
@
\end{subfigure}
