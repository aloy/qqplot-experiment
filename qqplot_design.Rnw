...
\newcommand{\hh2}[1]{{\color{orange} #1}}
...
text <- data.frame(label = c("Standard", "Detrended (1)", "Detrended (2)", "Control"), 
                   x=c(1.5, 3.5, 5.5, 7), 
                   y=30,
                   nu="df: 10",
                   s= "sample size n: 20")
...
for (fname in dt1) {
  ...
  outline1 <- data.frame(xmin=min(dframe$naive1.qq.x)*1.0, xmax=max(dframe$naive1.qq.x)*1.0, 
            ...
            .sample=dframe$.sample_outer[nrow(dframe)])  
  xlabel <- sprintf("De-trended~(1)~lineup~with~DH~band:~t[%d]~sample~of~size~%d", dframe$df[1], dframe$n[1])
  ...
  outline2 <- data.frame(xmin=min(fit2$ts.qq.x)*1.0, xmax=max(fit2$ts.qq.x)*1.0, 
            ...
            .sample=fit2$.sample_outer[nrow(fit2)])  
  xlabel <- sprintf("De-trended~(1)~lineup~with~TS~band:~t[%d]~sample~of~size~%d", dframe$df[1], dframe$n[1])
