...
In the lineup on the left the data sample (in panel \#$(2^2+1)$) is only identified once out of 21 times in the lineup showing the de-trended design \hh{with a DH band}. Observers instead pick panel \#$(3^2-2)$ 18 times. \hh{The lineup on the right shows the same data in the de-trended design using the TS confidence band.}}
\end{figure}
\afterpage{\clearpage}
