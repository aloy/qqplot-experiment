...
<<compare-designs, echo=FALSE, fig.width=10, fig.height=8, warning=FALSE, message=FALSE>>=
...
ggplot() + 
  ...
  geom_text(aes(x = x, y= y, label=label), size=7, colour="grey40", angle=90, hjust=0, data=text) + 
  geom_point(aes(x=as.numeric(CIdesign), y=100*correct/n, colour=factor(rep), shape=factor(signif)), size=4,  data=dataset) +
  geom_line(aes(x=as.numeric(CIdesign), y=100*correct/n, colour=factor(rep), group=data_name), data=dataset) +
  facet_grid(facets=nu~s, labeller="label_parsed") + scale_colour_brewer(palette="Set1") +
  theme_bw() + scale_shape_manual(values=c(1,19)) + ylab("Percentage of data identifications") +
#  scale_x_continuous(breaks=1:7, labels=levels(dataset$CIdesign)) + 
  scale_x_continuous("", breaks=1:7, labels=c("\nHD", "\nTS","HD", "TS","HD", "TS","Control")) +
