...
\hh{In only \Sexpr{round(length(grep("5_", turkall$choice_reason))/nrow(turkall)*100, 1)}\% of all evaluations made use of the option of giving feedback for the reason of their choice in form of a free-form answer. }
%
...
Under both the standard design and the detrended (1)  design there are \Sexpr{length(dt1)} lineups each with different outcomes. Under the detrended (2) design there are \Sexpr{length(dt2)} lineups with different outcomes. They are shown in figures~\ref{fig:lpsstds},~\ref{fig:lpsdt1}, and~\ref{fig:lpsdt2}, respectively. 
...
back1$ymax <- 105
  ...
  p1 <- rot2_lineup(dframe) + geom_text(aes(x=xdh, y=ydh, label=sprintf("%.1f/%.0f", Freq,max)), colour="grey70", size=7, data=dh, vjust=0, hjust=1)
  ...
  p2 <- rot2_ts_lineup(fit2) + geom_text(aes(x=xdt, y=ydt, label=sprintf("%.1f/%.0f", Freq, max)), colour="grey70", size=7, data=dt, vjust=0, hjust=1)
...
@
\caption{\label{fig:lpsdt1} De-trended (1) lineup with different outcomes depending on the choice of confidence band.  \hh{XXX I still want to show the parameter values somehow and the panel of the sample} }
\end{figure}
\afterpage{\clearpage}
