...
Based on visual inspection of a Q-Q plot, a
 ...
 whether the points fall inside the envelope of 95\%  pointwise confidence intervals \citep[][p.~150--154]{Davison:1997}. \al{Alternatively, simultaneous confidence intervals could be employed so that a rejection of normality would occur if any of the points fell outside of the interval. This is the approach taken by \citet{buja:2013} with their Tail-Sensitive (TS) simultaneous confidence intervals, which they found to be more powerful than the KS test to detect heavy-tailed distributions.
}
...
%\hhnote{ok, let's go with ordinary De-trended plots for the ones that are labelled De-trended (1) - i.e. full amount of space on the y-axis, and {\it adjusted De-trended plots} for the plots, in which we keep distances in x and y the same (De-trended (2)). I will start to make the changes throughout the paper. Those changes will be in orange.}
...
Examples of \hh{all the different} Q-Q plot \hh{designs} under consideration are displayed in Figure~\ref{qqplots}, and include (from left to right): a \emph{control} Q-Q plot, a \emph{standard} Q-Q plot with \hh{an added grey band representing a confidence band, a\HH{n \emph{adjusted}} \emph{de-trended} Q-Q plot preserving relative distances in the plot and a\HH{n \emph{ordinary}} \emph{de-trended} Q-Q plot making full use of the space along the $y$-axis.}
\hh{Q-Q plots in the upper row of figure~\ref{qqplots} use a Davison-Hinkley (DH) 95\% pointwise confidence region \citep{Davison:1997}, while the shading in the lower row represents 95\% Tail-Sensitive (TS) simultaneous confidence bands proposed by \cite{buja:2013}. }
...
\al{In Section~\ref{sec:power2} we compare the power of the lineup test of normality to the classical normality tests
 ...
 We conclude with a discussion and an outline of areas for future research.}
...
\hh{In order to evaluate the power of Q-Q plots and compare their performance to tests of normality, we make use of a simulation study 
...
These lineup \hh{data sets} are rendered in each of the Q-Q plot variations, resulting in $48 \times 7 = 336$ different lineups.  Additionally, standard plots (with both TS and DH bands) are rendered against $N(0, S^2)$ null data (see discussion below), adding another $2 \times 48 = 96$ lineups.
}
...
\HH{Null samples are drawn from $N(0, S^2)$; the reference line has a slope of $S$ (and an intercept of 0).  All panels have the same scale. Throughout the paper we use $\hat{\sigma}^2$ to indicate the adjusted sample variance $\hat{\sigma}^2 = (n-1)^{-1} \Sigma_{i=1}^n (x_i - \bar{x})^2$, whereas $\hat{S}^2$ is a more robust estimator based on the interquartile range.}
\end{itemize}
...
Using  Amazon MTurk \citep{amazon}, \Sexpr{length(unique(turk$ip_address))} independent observers were recruited and asked to evaluate ten lineups each. \hh{Lineups were randomly assigned to participants, such that each participant
...
%
\hh{Table~\ref{tab:coefs} gives an overview of the coefficients.}

\hh{As could be seen in various figures before, an increase in signal strength (larger sample size, fewer degrees of freedom) result in lineups, in which the sample is picked out more often by observers.}
...
Surprisingly, \HH{ordinary} de-trended Q-Q plots \HH{do not improve participants' probabilities } %is significantly less powerful 
in detecting non-normality. \HH{With DH bands, ordinary detrended Q-Q plots are actually performaing significantly worse than the control charts.}
...
\HH{In contrast to that, the adjusted version of detrended Q-Q plot shows a significant improvement in detecting non-normality over control and standard charts. }
%\hh{The first de-trended version of Q-Q plots, in which the aspect ratio is kept the same as in the standard plot, shows significant increases in detecting non-normality. }
 \hh{It seems that the additional space on the $y$-axis emphasizes artifacts \HH{in the ordinary design} that are hard to judge, because distances in the $x$ direction have a different meaning than distances in the $y$ direction. %Due to the difference in the aspect ratio, plotted distances between points are not proportional to distances in the data space.
 } 

\hh{For the other two designs, Q-Q plots with TS bands have slightly less power than the same version of Q-Q plots with DH bands. While  this effect is not significant, it is likely due to TS bands being wider in the center of the distribution and at the tips of the tails.}
...
%
\HH{ord.\ }detrended  (DH) & \bf -0.42 & \bf -0.61 & \bf  -0.24 && \bf 0.66 & \bf 0.54 & \bf  0.79 \\ 
\HH{ord.\ }detrended (TS) & 0.03 & -0.19 & 0.25 && 1.03 & 0.83 & 1.28 \\ [2pt]
%
\HH{adj.\ }detrended  (DH) & \bf 0.42 & \bf 0.20 & \bf 0.64 && \bf  1.52 & \bf 1.22 & \bf 1.89 \\ 
\HH{adj.\ }detrended  (TS) & \bf 0.32 & \bf 0.10 & \bf 0.53 && \bf 1.37 & \bf 1.10 & \bf 1.70 \\ [3pt]
...
To further investigate the difference between the designs, consider Figure \ref{fig:rotstd}. Here, we consider a sample from the $t_2$ distribution of size $n=20$ that is rejected \hh{from all designs besides the \HH{ord.\ detrended}  Q-Q plots with a DH band.}  
\hh{The \HH{ord.\ detrended}  DH band is shown on the left side of Figure~\ref{fig:rotstd}. Instead of the data sample in panel \#$(2^2+1)$ observers pick panel \#7 in 32.2 out of 45 evaluations. For any of the other designs, as e.g.\ the \HH{ord.\ detrended}  Q-Q plot with TS bands on the right of Figure~\ref{fig:rotstd}, observers pick the panel of the data sample at a rate that leads to a rejection of normality.  }
...
In the lineup on the left the data sample in panel \#$(2^2+1)$ is only identified \hh{2.8 times in 45} evaluations in the lineup showing the \HH{ord.\ detrended}  design \hh{with a DH band}. Observers instead pick panel \#$(3^2-2)$ \hh{32.2} times. \hh{The lineup on the right shows the same data in a \HH{ord.\ detrended}  Q-Q plot design using the TS confidence band. The data sample is picked 7.2 times in 27 evaluations, leading to a rejection of normality at a 5\% signficance level.}}
\end{figure}
\afterpage{\clearpage}
...
\hh{Only \Sexpr{round(length(grep("5_", turkall$choice_reason))/nrow(turkall)*100, 1)}\% of all evaluations made use of the option of giving feedback for the reason of their choice in form of a free-form answer. }
%
\hh{The two participants who gave a free-form answer for their choice in the lineup on the left hand side of Figure~\ref{fig:rotstd}, picked panel \#7 from the \HH{ordinary} de-trended lineup, and stated that this panel was the one with the ``most dots outside the shaded area,'' i.e.~they focused on the middle of panel \#7}. 
...
for (fname in dt2[-1]) {
  ...
  outline1 <- data.frame(xmin=min(dframe$naive1.qq.x)*1.0, xmax=max(dframe$naive1.qq.x)*1.0, 
            ...
            .sample=dframe$.sample_outer[nrow(dframe)])  
  xlabel <- sprintf("Detrended~(2)~lineup~with~DH~band:~t[%d]~sample~of~size~%d", dframe$df[1], dframe$n[1])
  ...
  outline2 <- data.frame(xmin=min(fit2$ts.qq.x)*1.0, xmax=max(fit2$ts.qq.x)*1.0, 
            ...
            .sample=fit2$.sample_outer[nrow(fit2)])  
  xlabel <- sprintf("De-trended~(2)~lineup~with~TS~band:~t[%d]~sample~of~size~%d", dframe$df[1], dframe$n[1])
...
\alnote{I have concerns here about BW printing. Could we adjust the saturation on one of the bins somehow? }
\begin{figure}[hbt]
\centering
