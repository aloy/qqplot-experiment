...
library(reshape2)
lppvalscb <- dcast(lppvals, ad+cvm+ks+lf+sw+n+df+rep~null, value.var="Standard")
...
p <- ggplot() + 
  ...
  geom_point(aes(x=Standard, y=sw, colour=df, shape=df), size=3, data=lppvalscb, alpha=0.9) + 
  scale_colour_brewer("degrees of\nfreedom", palette="Set2") + theme_bw() +scale_shape("degrees of\nfreedom") + ylim(c(-0.02,1)) + geom_point(aes(x=Standard, y=sw), pch=21, size=7, data=subset(lppvalscb, (Standard < 0.02 & sw > 0.2) | (Standard > 0.1 & sw < 0.05))) + theme(legend.position="bottom") + ylab("Shapiro-Wilks (p-values)") + xlab("Standard Q-Q plot lineup (estimated p-values)") 
