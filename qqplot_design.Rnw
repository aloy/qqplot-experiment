...
@
\caption{\label{fig:lpsdt1} De-trended (1) lineup with different outcomes depending on the choice of confidence band. The outlined panel marks the location of the data sample, the numbers at the bottom right are showing the number of votes each of the panels was picked by observers.   \hh{XXX I still want to show the parameter values somehow } }
\end{figure}
\afterpage{\clearpage}
