...
%\al{In this paper we refer to such a detrended Q-Q plot as Detrended (1).}
...
\alnote{Should we say more than this about MTurk?}
\hhnote{Let's leave it at that}
...
\alnote{Can we add the numbers in the background here like we did in the appendix?}
\hhnote{good idea!}
\begin{figure}[hbt]
%\begin{subfigure}{0.5\textwidth}
