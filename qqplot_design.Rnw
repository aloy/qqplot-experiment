...
\hh{Lineups of data samples from a $t_2$-distribution (bottom row)  are all rejected in lineups under the standard design, while the $t_{10}$ samples largely go undetected except for two of the larger samples.} 
