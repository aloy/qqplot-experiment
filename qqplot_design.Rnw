...
In Section~\ref{sec:power2} we will return to these tests in order to \hh{compare their effectiveness to visual evaluation of normality in Q-Q plots.}
%assess the effectiveness of different variations of standard Q-Q plots.
...
\hh{Include discussion of Tail-Sensitive (TS) simultaneous confidence bands \citep{buja:2013} here.} 
...
\hh{In the following sections of this paper we compare the power of classical tests in detecting deviations from normality compared to visual detecting using Q-Q plots (section~\ref{sec:power2}) based on a simulation study, which we introduce in section~\ref{sec:simu}. Besides the power comparison of classical versus visual tests, we compare the power of the different designs in section~\ref{sec:power1} in a generalized linear mixed model. We also explore the feedback of the independent observers to compare the rationale for plot selection (i.e., rejection of normality). }
...
\begin{itemize}
\item  evaluated each data set at most once, 
\item was exposed to multiple parameter settings (`harder' and `easier' lineups), and
\item evaluated each Q-Q plot design at most twice.
\end{itemize}
