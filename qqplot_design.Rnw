...
For the visual evaluation, we make use of Q-Q plots in a lineup test: each sample is randomly inserted among a set of 19 null plots constructed from normally distributed samples of the same size. For each data sample, two sets of null plots are created, so that we have a total of 48 lineup data. 
}
...
\begin{figure}[ht]
\centering
<<compare-designs, echo=FALSE, fig.width=12.5, fig.height=8, out.width='\\textwidth', warning=FALSE, message=FALSE>>=
...
dataset$pval <- NULL
require(vinference)
for (i in 1:nrow(dataset)) dataset$pval[i] <- with(dataset[i,], pV(x=correct, K=n, m=20, scenario=3))
...
dataset$s <- paste("s:", dataset$sample_size)
dataset$nu <- paste("nu:", dataset$df)
dataset$nu <- factor(dataset$nu, levels=c("nu: 10","nu: 5","nu: 2"))
...
text <- data.frame(label = c("Standard", "Detrended", "Detrended 2", "Control"), 
                   x=c(1.5, 3.5, 5.5, 7), 
                   y=30,
                   nu="nu: 10",
                   s= "s: 20")
...
ggplot() + 
  ...
  geom_point(aes(x=as.numeric(CIdesign), y=100*correct/n, colour=factor(rep), shape=factor(signif)), size=3,  data=dataset) +
  geom_line(aes(x=as.numeric(CIdesign), y=100*correct/n, colour=factor(rep), group=data_name), data=dataset) +
  facet_grid(facets=nu~s, labeller="label_parsed") + scale_colour_brewer("Replicate", palette="Set1") +
  theme_bw() + scale_shape_manual("Significant difference\nfrom normality",values=c(1,19)) + ylab("Percentage of data identifications") +
#  scale_x_continuous(breaks=1:7, labels=levels(dataset$CIdesign)) + 
...
Lineups showing a sample from a $t_5$ distribution \hh{show the most variability. Table~\ref{tab:no1} gives an overview of the rejections of a standard normal distribution by design. A more thorough discussion on the power of different designs follows in section~\ref{sec:power1}.} 
 
\begin{table} 
\centering
\begin{tabular}{lccccccc}\hline
\bf Design & \multicolumn{2}{c}{\bf Standard} & \multicolumn{2}{c}{\bf De-trended (1)} & \multicolumn{2}{c}{\bf Detrended (2)} & \bf Control \\
\bf Confidence & HD & TS & HD & TS & HD & TS & --- \\ \hline
...
\caption{\label{tab:reject}
 From left to right, we see the number of rejections from visual inference as well as the  Shapiro-Wilk, Anderson-Darling, Lilliefors,  Kolmogorov-Smirnov, and Cram\'er-von Mises tests for normality. Out of the 24 non-normal samples, 12 get rejected at the 5\% significance level based on evaluation by observers in the standard lineup with Davison-Hinkley confidence bands, 15 get rejected using lineups of standard Q-Q plots with TS confidence bands . None of the standard normal tests come close to those rejection rates. The power we observe here matches the power discussion by \citet{razali:2011} for the SW, AD, and the LF  test. The number in parentheses is the number of situations in which the Standard Q-Q plot agrees with the normal test in rejecting normality of the sample. \hh{XXX we should do something about the parentheses}}

\begin{tabular}{rrrrrrrrr}
  \hline
 & Standard (DH/TS) & SW & AD & LF   & CvM & \\ 
  \hline
  \hline
%  reject $N(0,1)$ & 15 & 14 & 12  &   &  &   &  & 2\\ 
%  not reject & 19 & 20 & 22 &  &   &  &   & 44\\ 
%   \hline
  reject $N(0,S^2)$  & 14 / 17 &  8 (7) & 5 (5) &  5 (5) & 4 (4) & \\ 
...
pm12 <- dcast(pm, n+df+variable~rep, value.var="value")
pm12$`1` <- as.numeric(pm12$`1`)
pm12$`2` <- as.numeric(pm12$`2`)
pm12$min <- pmin(pm12$`1`, pm12$`2`)
pm12$max <- pmax(pm12$`1`, pm12$`2`)

pm <- melt(pm12, id.var=c("n", "df", "variable"), measure.var=c("min", "max"))
names(pm)[4] <- "rep"
pm$value <- as.numeric(pm$value)
pm$cval <- cut(pm$value, breaks=c(0, 0.001, 0.01, 0.05, 0.1, 0.2, 0.5, 1), right=FALSE)
pm$variable <- factor(pm$variable, levels=c("StdTS.pvals", "StdDH.pvals", "sw", "ad", "lf", "cvm"))
levels(pm$variable) <- c("Std\nTS", "Std\nDH", "SW", "AD", "LF", "CvM")
levels(pm$rep) <- c(1,2)
pm$nlab <- paste("n:",pm$n)
pm$dflab <- factor(paste("nu:",pm$df))
pm$dflab <- factor(pm$dflab, levels=c("nu: 10", "nu: 5", "nu: 2"))

qplot(variable, rep, fill=cval, data=pm, geom="tile", colour=I("grey80")) + 
  facet_grid(dflab~nlab, labeller="label_parsed") +  theme_bw() + 
  scale_fill_brewer("p-value", palette="RdBu") + xlab("") + ylab("") +
  geom_vline(xintercept=2.5, colour="white", size=2) + 
  theme(legend.position="bottom") + 
  geom_point(aes(variable, rep), data=subset(pm, value < 0.05), size=8, pch=4, show_guide=FALSE)
@
\caption{\label{fig:pvals} Comparison of visual tests and common normality tests. Each panel shows the outcome for the two samples of size $n$ drawn from a $t_\nu$ distribution. If normality of a sample is rejected by a test, the tile is crossed out. Shades of red all indicate rejections, blue shades indicated non-rejections. Color intensity reflects levels of significance.}
\end{figure}
\afterpage{\clearpage}
...
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
<<notrotated3, dependson='data', echo=FALSE, fig.width=6, fig.height=6>>=
  fit2 <- dframe
...
% \begin{figure}[hbt]
% <<lps-dsn2, dependson='functions', echo=FALSE, message=FALSE>>=
% dframe <- read.csv("lineup-data/data-1-2-15-50-2-13-6.csv")
% dframe$.sample <- dframe$.sample_outer
% 
% fit2 <- dframe
...
% \end{subfigure}
% \caption{\label{fig:lps} Lineup plots of standard Q-Q plots. The observed data is the same, but  reference lines and envelopes are based on a standard normal distribution on the left; while  reference lines and envelopes for the lineup on the right are based on a normal distribution $N(0, \widehat{S}^2)$, where $\widehat{S}$ is based on the IQR of the observed data.
% The observed data in both lineups is displayed in panel \#$(3^2 - 3)$\footnote{The little piece of calculus is imposing a small cognitive barrier that allows the reader to evaluate the lineup once without already being biased by knowing the location of the data panel.}. }
% \end{figure}
% \afterpage{\clearpage}
