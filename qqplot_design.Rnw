...
ggplot() + 
  geom_rect(aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), 
            fill="grey70", alpha=0.5, data=back1) + 
  geom_rect(aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax), 
            fill="grey70", alpha=0.5, data=back2) + 
  geom_text(aes(x = x, y= y, label=label), size=5, colour="grey40", 
            angle=90, hjust=0, data=text) + 
  geom_point(aes(x=as.numeric(CIdesign), y=100*correct/n, colour= C, 
                 shape=C), size=3.5,  data=dataset) +
  geom_line(aes(x=as.numeric(CIdesign), y=100*correct/n, colour=C, g
                roup=data_name), data=dataset) +
  facet_grid(facets=nu~s) +
  theme_bw()  + 
  scale_shape_manual("Significant difference\nfrom normality", 
                     values=c(1,16, 2, 17)) +
  scale_colour_manual("Significant difference\nfrom normality", 
                      values=c("#e41a1c", "#e41a1c", "#377eb8", "#377eb8")) +
  ylab("Percentage of data identifications") +
  scale_x_continuous("", breaks=1:7, labels=c("\t\t\t\t\tStandard\nDH", "\nTS","\t\t\t\t\tadj. Detrended\nDH", "\nTS","\t\t\t\t\tord. Detrended\nDH", "\nTS","Control")) +
  theme(legend.position="bottom", legend.direction="horizontal", 
        legend.box="horizontal", legend.key.width =unit(1, "cm")) + 
  guides(shape=guide_legend(title="Significant difference\nfrom normality", 
                            ncol=2))
@
