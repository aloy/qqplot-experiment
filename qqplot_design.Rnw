...
\bf Confidence & HD & TS & HD & TS & HD & TS & --- \\ \hline \hline
...
ggplot() +
  ...
  facet_grid(dflab~nlab, labeller="label_parsed") +  theme_bw() + xlab("") + ylab("p-value") +
  theme(legend.position="bottom") + 
...
%
Detrended 1 (HD) & \bf 0.42 & \bf 0.20 & \bf 0.64 && \bf  1.52 & \bf 1.22 & \bf 1.89 \\ 
  Detrended 1 (TS) & \bf 0.32 & \bf 0.10 & \bf 0.53 && \bf 1.37 & \bf 1.10 & \bf 1.70 \\ [2pt]
%
Detrended 2 (HD) & \bf -0.42 & \bf -0.61 & \bf  -0.24 && \bf 0.66 & \bf 0.54 & \bf  0.79 \\ 
...
\hh{The de-trended (2) DH band is shown on the left side of figure~\ref{fig:rotstd}. Instead of the data sample in panel \#$(2^2+1)$ observers pick panel \#7 in 18 out of 21 evaluations. For any of the other designs, as e.g.\ the de-trended (2) Q-Q plot with TS bands on the right of figure~\ref{fig:rotstd}, observers pick the panel of the data sample at a rate that leads to a rejection of normality for all of them.  }
...
In the lineup on the left the data sample in panel \#$(2^2+1)$ is only identified once in 21 evaluations in the lineup showing the de-trended (2) design \hh{with a DH band}. Observers instead pick panel \#$(3^2-2)$ 18 times. \hh{The lineup on the right shows the same data in the same de-trended (2) design using the TS confidence band.}}
\end{figure}
\afterpage{\clearpage}
...
%Two of the observers picking panel \#7 from the de-trended lineup thought that this panel was the one with the ``most dots outside the shaded area,'' i.e.~they focused on the middle of plot \#7. This is not a singular occurrence---when investigating  overall reasoning we took a closer look at the effect of the  words ``area'' and ``outside'' in the reason participants gave for making their choice of plot (figure~\ref{fig:rotfalse}). As expected, these words barely occur in the control design (where there is no shaded area).
...
trt <- ddply(turkall, .(CIdesign, correct), summarise,
      n=sum(weight),
...
trtm <- melt(trt,id.var=c("CIdesign", "n","choice"), measure.var=c("outlier", "left", "right", "curve", "other"))
trtm$pct <- trtm$value/trtm$n*100


trt$otherw <- gsub(",","", trt$otherw)
trt$otherw <- gsub(" +"," ", trt$otherw)
trt$otherw <- tolower(trt$otherw)
...
% trtm2 <- melt(trt,id.var=c("CIdesign", "n","choice"), measure.var=c("outlier", "left", "right", "curve"))
% qplot(CIdesign, fill=choice, weight=value, data=trtm2, position="dodge") + scale_fill_brewer(palette="Set2") + facet_wrap(~variable) + theme_bw()
% @
...
back1 <- data.frame(expand.grid(variable=levels(trtm$variable)))
back1$xmin <- 1.5
back1$xmax <- 3.5
back1$ymin <- -5
back1$ymax <- 75
